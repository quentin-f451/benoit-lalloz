<div class="grid__image grid__image--<?= $mediaArray['orientation'] ?> js-thumb">
	<?php if($mediaArray['status'] == 'listed') echo '<a href="' . $mediaArray["url"] . '">'; ?>
		...
		<img class="thumb thumb--video <?php if($media != $first) echo 'thumb--hidden' ?>" src="<?= $media['srcXLarge'] ?>"
			srcset="<?= $media['srcXSmall'] ?> 400w, <?= $media['srcSmall'] ?> 750w, <?= $media['srcMedium'] ?> 1000w, <?= $media['srcLarge'] ?> 1720w, <?= $media['srcXLarge'] ?> 2560w"
